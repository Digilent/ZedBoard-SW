...
int main()
{
	...
	{//Constructor of hardware driver classes might throw
		...
		// Power cycle all connected cameras
		// This is done once here instead of passing each camera a valid GPIO handle to power cycle itself,
		// because the FMC Pcam Adapter board hardwires all cameras' pwups together.
		...
		// select the first connected camera as the appointed master
		// For more information on synchronization, check out the demo's reference page,
		// section "Implementation details: Notes on VDMA synchronization"
		if(cam_a_ptr){
			master_select = 0;
		}
		else if(cam_b_ptr){
		...
		if(cam_a_ptr){
			try{
		     		input_pipeline_mode_change(vdma_a_driver, *cam_a_ptr, scaler_a, Resolution::R960_540_60_PP, Resolution::R1920_1080_60_PP, OV5640_cfg::mode_t::MODE_1080P_1920_1080_30fps, XPAR_MIPI_D_PHY_RX_A_S_AXI_LITE_BASEADDR, XPAR_MIPI_CSI_2_RX_A_S_AXI_LITE_BASEADDR, XPAR_AXI_GAMMACORRECTION_A_BASEADDR, master_select, master_select == 0);
			}
		 ...
		 if(cam_b_ptr){
			try{
				input_pipeline_mode_change(vdma_b_driver, *cam_b_ptr, scaler_b, Resolution::R960_540_60_PP, Resolution::R1920_1080_60_PP, OV5640_cfg::mode_t::MODE_1080P_1920_1080_30fps, XPAR_MIPI_D_PHY_RX_B_S_AXI_LITE_BASEADDR, XPAR_MIPI_CSI_2_RX_B_S_AXI_LITE_BASEADDR, XPAR_AXI_GAMMACORRECTION_B_BASEADDR, master_select, master_select == 1);
			}
			catch(std::exception const& e){
		 ...
		 if(cam_c_ptr){
			try{
				input_pipeline_mode_change(vdma_c_driver, *cam_c_ptr, scaler_c, Resolution::R960_540_60_PP, Resolution::R1920_1080_60_PP, OV5640_cfg::mode_t::MODE_1080P_1920_1080_30fps, XPAR_MIPI_D_PHY_RX_C_S_AXI_LITE_BASEADDR, XPAR_MIPI_CSI_2_RX_C_S_AXI_LITE_BASEADDR, XPAR_AXI_GAMMACORRECTION_C_BASEADDR, master_select, master_select == 2);
			}
			catch(std::exception const& e){
		 ...
		 if(cam_d_ptr){
			try{
				input_pipeline_mode_change(vdma_d_driver, *cam_d_ptr, scaler_d, Resolution::R960_540_60_PP, Resolution::R1920_1080_60_PP, OV5640_cfg::mode_t::MODE_1080P_1920_1080_30fps, XPAR_MIPI_D_PHY_RX_D_S_AXI_LITE_BASEADDR, XPAR_MIPI_CSI_2_RX_D_S_AXI_LITE_BASEADDR, XPAR_AXI_GAMMACORRECTION_D_BASEADDR, master_select, master_select == 3);
			}
			catch(std::exception const& e){
		...
		output_pipeline_mode_change(vdma_a_driver, vid, Resolution::R1920_1080_60_PP, master_select);
...
void input_pipeline_mode_change(AXI_VDMA<ScuGicInterruptController>& vdma_driver, OV5640& cam, Scaler& scaler, Resolution HW_ScaledCaptureRes, Resolution VideoOutputRes, OV5640_cfg::mode_t mode, uintptr_t dphy_baseaddr, uintptr_t csi2_baseaddr, uintptr_t gamma_baseaddr, u8 master_select, bool is_master)
{
	...
	{
		vdma_driver.configureWrite(timing[static_cast<int>(HW_ScaledCaptureRes)].h_active, timing[static_cast<int>(HW_ScaledCaptureRes)].v_active, timing[static_cast<int>(VideoOutputRes)].h_active, timing[static_cast<int>(VideoOutputRes)].v_active, master_select, is_master);
